    ...
    const getRow = (t) => {
        return t.rows.map((row, index) => {
            return [
                ...
                { value: "", field: row.field, required: row.required, datatype: row.datatype, dataEditor: (row.field === ENUMS.FORM_FIELDS.PAD_INFO.REP_CONTACT_NUMBER || row.field === ENUMS.FORM_FIELDS.JOB_INFO.AFE_ID) ? NumberInput : null },
            ]
        })
    }
