...
export default function TrackingSheet() {
  ...
  const [
    ...
    setPerforationIntervalInformationValues,
  ] = useState({
    acid: null,
  ...
  const addItem = () => {
    setItems([
      ...items,
      { value: items.length + 1, label: `Stage ${items.length + 1} (Added)` },
    ]);
  };
  ...
  const perforationIntervalInformationChange = (
    changedValue,
    newFormValues
  ) => {
  ...
  const handleTrackingSheetSubmit = async () => {
    ...
    try {
      if (isUpdating) {
        const stageTrackingPresent = stageSheetList.find(
          (s) => s.stage_n === Number(selectedStage)
        );
        const addedProppantData = proppantFormValues.proppantData.filter(
          (item) => {
        ...
        await projectApi.putUpdateTrackingSheet(
          stageTrackingPresent.uuid,
          updatedTrackingSheet
        );
  ...
  const populateFormData = useCallback(
    (trackingSheetData) => {
      const {
        ...
        FormDataSerializer.trackingSheetPopulateDataSerializer(
          trackingSheetData
        );
      dynamicFormNestItemForm.setFieldsValue(dynamicFormNestItemValuesData);
      perforationIntervalInformationForm.setFieldsValue(
        perforationIntervalInformationValuesData
      );
  ...
  const fetchStagesSubmitted = useCallback(
    async (well_id, projectData) => {
      const stages = getStages(
        projectData.wells.find((well) => well.uuid === well_id).num_stages
      );
      try {
        ...
        if (data.stages.length > 0) {
          const stageTrackingPresent = data.stages.find(
            (s) => s.stage_n === stages[Number(selectedStage) - 1].value
          );
  ...
  const resetForm = useCallback(() => {
    dynamicFormNestItemForm.setFieldsValue(
      FormInitialValues.dynamicFormNestItemValues
    );
    perforationIntervalInformationForm.setFieldsValue(
      FormInitialValues.perforationIntervalInformationValues
    );
  ...
  useState(() => {
    if (
      project &&
      locationData.pathname === ENUMS.ROUTES.ADMIN + ENUMS.ROUTES.TRACKING_SHEET
    ) {
  ...
  useEffect(() => {
    return history.listen((location) => {
      if (
        location.pathname ===
        ENUMS.ROUTES.ADMIN + ENUMS.ROUTES.TRACKING_SHEET
      ) {
                            ...
                            <Fragment key={key}>
                              ...
                              <Col span={5}>
                                <StyledFormItem
                                  ...
                                  rules={[
                                    { required: true, message: "Bulk density" },
                                  ]}
                                  label="Bulk density"
                                  labelAlign="left"
                                  isVertical
                                      ...
                                      setRemovedFluidsAndProppant((prev) => {
                                        return {
                                          ...
                                          proppant_data_ids: [
                                            ...
                                            proppantFormValues.proppantData[
                                              name
                                            ].id,
                                          ],
                                        };
